...
export class StudentController {
  ...
  async getDashboard(@Req() req: any) {
      ...
      if (masterCheck.length > 0 && masterCheck[0].status == 1) {
        if (masterCheck[0].is_premium == 0 || fid == 1) {
          ...
          const addonCheck = await this.dataSource.query(
            "SELECT addon_xp_coupons FROM franchises WHERE id=?", [fid]
          );
    ...
    let xpData: any = { current_xp: 0, job_xp: 0, current_level: 1, current_streak: 0, last_login_date: null, last_task_date: null, spin_date: null };
    if (isGamified) {
      try {
        const xpRows = await this.dataSource.query(
          'SELECT * FROM student_gamification WHERE student_id = ?', [userId]
        );
        if (xpRows.length > 0) {
          xpData = xpRows[0];
        } else {
    ...
    // Rank calculation
    let skillRank = 'Rookie', rankColor = '#94a3b8';
    if (sps >= 100) { skillRank = 'Bronze'; rankColor = '#b45309'; }
    if (sps >= 500) { skillRank = 'Silver'; rankColor = '#94a3b8'; }
    if (sps >= 1500) { skillRank = 'Gold'; rankColor = '#eab308'; }
    if (sps >= 3000) { skillRank = 'Platinum'; rankColor = '#3b82f6'; }
    if (sps >= 5000) { skillRank = 'Diamond'; rankColor = '#8b5cf6'; }
    ...
    let jobCount = 0, netHired = 0, pendingInvites = 0, totalInvites = 0;
    if (!isSchool) {
      try {
        const jc = await this.dataSource.query("SELECT COUNT(*) as total FROM jobs");
        jobCount = jc[0]?.total || 0;
        const hc = await this.dataSource.query("SELECT COUNT(*) as total FROM job_applications WHERE LOWER(status)='accepted'");
        netHired = hc[0]?.total || 0;
        const ic = await this.dataSource.query("SELECT status FROM job_invites WHERE student_id=?", [userId]);
        totalInvites = ic.length;
        pendingInvites = ic.filter((r: any) => r.status?.toLowerCase() === 'invited').length;
      } catch (e) {}
    }
    ...
    let netStudents = 0;
    try {
      const sc = await this.dataSource.query("SELECT COUNT(id) as total FROM students WHERE status='Active'");
    ...
    let opId = student.enrollment_no || student.roll_no || student.id?.toString();
    ...
    return {
      ...
      data: {
        profile: {
          ...
          enrollment_no: student.enrollment_no || student.roll_no || student.id?.toString(),
          operative_id: opId,
          photo: photoUrl,
        ...
        notice: latestNotice ? { title: latestNotice.title, message: latestNotice.message } : null,
        notifications: notifications.map((n: any) => ({ id: n.id, title: n.title, message: n.message })),
        ...
        jobs: { count: Number(jobCount), hired: Number(netHired), pending_invites: Number(pendingInvites), total_invites: Number(totalInvites) },
  ...
  async getNotifications(@Req() req: any) {
    ...
    try {
      notifications = await this.dataSource.query(
        "SELECT * FROM student_notifications WHERE student_id=? ORDER BY id DESC LIMIT 20", [userId]
      );
    } catch (e) {}
  ...
  async getChatMessages(@Req() req: any) {
    ...
    try {
      ...
      // Mark is_me
      messages = messages.map((m: any) => ({ ...m, is_me: m.sender_id === userId }));
    } catch (e) {}
  ...
  async sendChatMessage(@Req() req: any) {
    ...
    if (!message || !message.trim()) throw new UnauthorizedException('Message is required');

    try {
      // Get franchise_id
      const student = await this.dataSource.query('SELECT franchise_id FROM students WHERE id = ?', [userId]);
  ...
  async getTalentFeedJobs(@Req() req: any) {
    ...
    try {
      ...
      `;
      const pending_invites = await this.dataSource.query(invitesQuery, [studentId]);

      const spsRes = await this.dataSource.query(`SELECT job_xp FROM student_gamification WHERE student_id=?`, [studentId]);
      ...
      return { success: false, message: 'Could not fetch jobs', error: error.message };
    }
  }
  ...
  async applyForJob(@Req() req: any, @Body() body: { job_id: number }) {
    ...
    try {
      const { job_id } = body;
      const jobRes = await this.dataSource.query(`SELECT req_sps FROM jobs WHERE id=?`, [job_id]);
      if (jobRes.length === 0) return { success: false, message: 'Job not found' };
      ...
      if (mySps < reqSps) {
        return { success: false, message: `SKILL LEVEL TOO LOW! You need at least ${reqSps} SPS.` };
      }

      const check = await this.dataSource.query(`SELECT id FROM job_applications WHERE job_id=? AND student_id=?`, [job_id, studentId]);
      if (check.length > 0) {
        return { success: false, message: 'You are already active on this mission.' };
      }

      await this.dataSource.query(`INSERT INTO job_applications (job_id, student_id, status) VALUES (?, ?, 'pending')`, [job_id, studentId]);
      return { success: true, message: 'Profile Transmitted! Employer is reviewing your skills.' };
    } catch (error) {
      return { success: false, message: 'Application failed', error: error.message };
    }
  }
  ...
  @UseGuards(JwtAuthGuard)
  async submitTypingScore(@Req() req: any, @Body() body: { wpm: number, accuracy: number }) {
    ...
    try {
      ...
      if (accuracy < 90) {
        return { success: false, message: 'Accuracy must be at least 90% to earn SPS!' };
      }
      ...
      return { success: true, message: `Awesome! You earned ${spsEarned} SPS & XP!`, earned: spsEarned };
    } catch (e) {
  ...
  async getSpinStatus(@Req() req: any) {
    ...
    try {
      ...
      return { success: true, data: { alreadySpun, current_xp: data.current_xp || 0 } };
    } catch (e) {
  ...
  async getLeaderboard(@Req() req: any) {
    const studentId = req.user.sub || req.user.userId || req.user.id;
    try {
      const studentRes = await this.dataSource.query(`SELECT batch_time FROM students WHERE id=?`, [studentId]);
      ...
      const myGlobalRank = globalLeaderboard.findIndex((s: any) => s.id === studentId) + 1;
      const myBatchRank = batchLeaderboard.findIndex((s: any) => s.id === studentId) + 1;
      ...
      return { success: false, message: 'Could not fetch leaderboard', error: error.message };
    }
  }
  ...
  async submitFeePayment(@Req() req: any, @Body() body: any) {
    ...
    if (!amount || !txn_id || !base64_screenshot) {
      return { success: false, message: 'Amount, Transaction ID, and Screenshot are required' };
    }
    ...
    try {
      ...
      return { success: true, message: 'Payment receipt submitted successfully! Admin will verify soon.' };
    } catch (e) {
      console.error(e);
  ...
  async updateProfile(@Req() req: any) {
    ...
    const { bio, github_link, linkedin_link, portfolio_link, base64_photo } = req.body;
    let updates: string[] = [];
    let params: any[] = [];

    if (bio !== undefined) { updates.push('bio=?'); params.push(bio); }
    if (github_link !== undefined) { updates.push('github_link=?'); params.push(github_link); }
    if (linkedin_link !== undefined) { updates.push('linkedin_link=?'); params.push(linkedin_link); }
    if (portfolio_link !== undefined) { updates.push('portfolio_link=?'); params.push(portfolio_link); }
    ...
    if (updates.length > 0) {
      params.push(userId);
      await this.dataSource.query(`UPDATE students SET ${updates.join(', ')} WHERE id=?`, params);
    }
  ...
  @UseGuards(JwtAuthGuard)
  async buyXpStoreItem(@Req() req: any, @Body() body: { item_name: string, cost: number }) {
    const studentId = req.user.sub || req.user.userId || req.user.id;
    try {
      ...
      if (currentXp < cost) {
        return { success: false, message: 'Not Enough XP! You need to grind more in the Arena to buy this.' };
      }
      ...
      if (item_name === 'Rs 100 Fee Discount') {
        ...
        if (discountsLeft <= 0) {
          return { success: false, message: 'Limit Reached 🔒 You have already claimed the maximum fee discount of Rs 600 this year.' };
        }
  ...
  async getTimetable(@Req() req: any) {
    const studentId = req.user.sub || req.user.userId || req.user.id;
    try {
      const studentRes = await this.dataSource.query(`SELECT course_id, franchise_id FROM students WHERE id=?`, [studentId]);
      if (!studentRes.length) return { success: false, message: 'Student not found' };
      ...
      return { success: false, message: 'Could not fetch timetable', error: error.message };
    }
  }
  ...
  async getSyllabus(@Req() req: any) {
    ...
    try {
      ...
      return { success: false, message: 'Could not fetch syllabus', error: error.message };
    }
  }
  ...
  async getDateSheet(@Req() req: any) {
    const studentId = req.user.sub || req.user.userId || req.user.id;
    try {
      const studentRes = await this.dataSource.query(`SELECT franchise_id FROM students WHERE id=?`, [studentId]);
      if (!studentRes.length) return { success: false, message: 'Student not found' };
      const franchise_id = studentRes[0].franchise_id;

      // Check which table exists (matching PHP logic)
      let tableName = '';
      for (const tbl of ['date_sheets', 'date_sheet_uploads', 'exam_date_sheets']) {
        const chk = await this.dataSource.query(`SHOW TABLES LIKE '${tbl}'`);
        if (chk.length > 0) { tableName = tbl; break; }
      }
      ...
      let datesheets = [];
      try {
        const hasFranchise = await this.dataSource.query(`SHOW COLUMNS FROM \`${tableName}\` LIKE 'franchise_id'`);
        if (hasFranchise.length > 0) {
          ...
          datesheets = await this.dataSource.query(`SELECT * FROM \`${tableName}\` ORDER BY id DESC`);
        }
      } catch (e) {
        // ignore
      }
      ...
      return { success: false, message: 'Could not fetch datesheets', error: error.message };
    }
  }
  ...
  async getHolidays(@Req() req: any) {
    ...
    try {
      ...
      return { success: false, message: 'Could not fetch holidays', error: error.message };
    }
  }
  ...
  async getIdCard(@Req() req: any) {
    ...
    try {
      ...
      const inst_name = student.franchise_name || student.name || student.branch_name || "ICT COMPUTER EDUCATION";
      const inst_addr = student.address || student.branch_address || student.location || "Main Campus, India";
      const inst_phone = student.phone || student.branch_phone || student.contact || "Support Team";
      
      const photo_url = (student.photo && student.photo !== 'null') ? student.photo : '';
      
      const roll_prefix = student.invoice_prefix ? student.invoice_prefix.replace(/-$/, '') : (student.fr_code || 'ICT');
      const display_id = student.roll_no || student.admission_no || (roll_prefix + '-' + student.id.toString().padStart(4, '0'));
      ...
      return { success: false, message: 'Could not fetch ID card details', error: error.message };
    }
  }
  ...
  async getResults(@Req() req: any) {
    ...
    try {
      ...
      return { success: false, message: 'Failed to fetch results', error: error.message };
    }
  }
  ...
  async getLibrary(@Req() req: any) {
    const studentId = req.user.sub || req.user.userId || req.user.id;
    try {
      const studentRes = await this.dataSource.query(`SELECT course_id FROM students WHERE id=?`, [studentId]);
      if (!studentRes.length) return { success: false, message: 'Student not found' };
      ...
      return { success: false, message: 'Failed to fetch library materials', error: error.message };
    }
  }
  ...
  async getActiveExams(@Req() req: any) {
    ...
    try {
      ...
      return { success: false, message: 'Failed to fetch active exams', error: error.message };
    }
  }
  ...
  async getQuiz(@Req() req: any) {
    try {
      let questions = [];
      try {
        questions = await this.dataSource.query(`SELECT * FROM daily_quiz_questions ORDER BY RAND() LIMIT 20`);
      } catch (e) {}
      
      return { success: true, data: questions };
    } catch (error) {
      return { success: false, message: 'Failed to fetch quiz questions', error: error.message };
    }
  }
  ...
  async getResume(@Req() req: any) {
    ...
    try {
      ...
      let gamification = { current_level: 1, current_xp: 0, job_xp: 0, best_wpm: 0 };
      try {
        const gRes = await this.dataSource.query(`SELECT * FROM student_gamification WHERE student_id=?`, [studentId]);
      ...
      return { success: false, message: 'Failed to fetch resume', error: error.message };
    }
  }
  ...
  async getTrophies(@Req() req: any) {
    const studentId = req.user.sub || req.user.userId || req.user.id;
    try {
      let gamification = { current_level: 1, current_xp: 0, job_xp: 0, current_streak: 0 };
      try {
        const gRes = await this.dataSource.query(`SELECT * FROM student_gamification WHERE student_id=?`, [studentId]);
        if (gRes.length) gamification = gRes[0];
      } catch (e) {}
      ...
      let quizCount = 0;
      try {
        const qRes = await this.dataSource.query(`SELECT COUNT(id) as total_won FROM xp_activity_log WHERE student_id=? AND activity_name='Daily Brain Quiz' AND xp_earned > 0`, [studentId]);
        if (qRes.length) quizCount = qRes[0].total_won || 0;
      } catch (e) {}

      let hasPurchase = false;
      try {
        const pRes = await this.dataSource.query(`SELECT id FROM student_purchases WHERE student_id=? LIMIT 1`, [studentId]);
        if (pRes.length) hasPurchase = true;
      } catch (e) {}

      let hasJob = false;
      try {
        const jRes = await this.dataSource.query(`SELECT id FROM job_applications WHERE student_id=? AND LOWER(status)='accepted' LIMIT 1`, [studentId]);
        if (jRes.length) hasJob = true;
      } catch (e) {}

      const badges = [
        { name: "Rookie Starter", desc: "Join the Shadow Arena.", icon: "military_tech", color: "0xFF10B981", unlocked: true },
        { name: "Quiz Whiz", desc: "Win the Daily Quiz 30 Times.", icon: "psychology", color: "0xFFBB86FC", unlocked: quizCount >= 30 },
        { name: "First Blood", desc: "Purchase your first Market Drop.", icon: "shopping_cart", color: "0xFFEF4444", unlocked: hasPurchase },
        { name: "Firestarter", desc: "Maintain a 30-Day Login Streak.", icon: "local_fire_department", color: "0xFFF59E0B", unlocked: gamification.current_streak >= 30 },
        { name: "Level 10 Boss", desc: "Reach Level 10 in Gamification.", icon: "stars", color: "0xFF3B82F6", unlocked: gamification.current_level >= 10 },
        { name: "Hired Professional", desc: "Get Hired for a Job.", icon: "work", color: "0xFFEC4899", unlocked: hasJob },
      ];

      return { success: true, data: { gamification, badges } };
    } catch (error) {
      return { success: false, message: 'Failed to fetch trophies', error: error.message };
    }
  }
  ...
  async getTimeline(@Req() req: any) {
    ...
    try {
      let logs = [];
      try {
        logs = await this.dataSource.query(`SELECT * FROM xp_activity_log WHERE student_id=? ORDER BY id DESC LIMIT 50`, [studentId]);
      } catch (e) {}
      return { success: true, data: logs };
    } catch (error) {
      return { success: false, message: 'Failed to fetch timeline', error: error.message };
    }
  }
  ...
  async getHelpdeskTickets(@Req() req: any) {
    ...
    try {
      let tickets = [];
      try {
        tickets = await this.dataSource.query(`SELECT * FROM student_queries WHERE student_id=? ORDER BY id DESC`, [studentId]);
      } catch (e) {}
      return { success: true, data: tickets };
    } catch (error) {
      return { success: false, message: 'Failed to fetch tickets', error: error.message };
    }
  }
  ...
  async submitHelpdeskTicket(@Req() req: any, @Body() body: any) {
    ...
    try {
      let franchise_id = 1;
      try {
        const sRes = await this.dataSource.query(`SELECT franchise_id FROM students WHERE id=?`, [studentId]);
      ...
      try {
        ...
        return { success: false, message: 'Error inserting ticket', error: e.message };
      }
      return { success: true, message: 'Ticket submitted successfully' };
    } catch (error) {
      return { success: false, message: 'Failed to submit ticket', error: error.message };
    }
  }
  ...
  async getBusTracking(@Req() req: any) {
    ...
    try {
      ...
      if (!busData) return { success: false, message: 'No bus assigned to your account.' };
      
      return { success: true, data: busData };
    } catch (error) {
      return { success: false, message: 'Failed to fetch bus details', error: error.message };
    }
  }
  ...
  async getReferral(@Req() req: any) {
    const studentId = req.user.sub || req.user.userId || req.user.id;
    try {
      let studentData = { name: 'Operative', custom_domain: '', subdomain: '', branch_name: '' };
      try {
      ...
      return { success: false, message: 'Failed to fetch referral details', error: error.message };
    }
  }
}

import { Controller, Get, Post, Body, Param, Req, UseGuards, HttpException, HttpStatus, Query } from '@nestjs/common';
...
export class DynamicController {
  ...
  async createRecord(
    ...
    try {
      const tableCheck = await this.dataSource.query(`SHOW TABLES LIKE ?`, [table]);
      if (tableCheck.length === 0) throw new HttpException('Table does not exist', HttpStatus.BAD_REQUEST);
      ...
      } else if (hasFranchise && franchiseId === 1 && !insertData.franchise_id) {
      ...
      if (Object.keys(finalData).length === 0) {
        throw new HttpException('No valid data provided', HttpStatus.BAD_REQUEST);
      }
      ...
      return { success: true, message: 'Record created successfully', insertId: result.insertId };
    } catch (e: any) {
      throw new HttpException(e.message, HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }

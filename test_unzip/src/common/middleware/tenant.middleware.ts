...
export class TenantMiddleware implements NestMiddleware {
  ...
  async use(req: Request, res: Response, next: NextFunction) {
    ...
    let resolvedDomain = (req.headers['x-tenant-domain'] as string) || req.headers.host || req.hostname;
      ...
      try {
        ...
        if (hostname !== 'localhost' && hostname !== '127.0.0.1') {
          ...
          if (franchiseData) {
            ...
            if (franchiseData.addon_resort_erp === 1 || plan.includes('resort')) moduleId = 'resort';
            else if (franchiseData.addon_institute_erp === 1 || plan.includes('institute')) moduleId = 'institute';
            else if (plan.includes('billing')) moduleId = 'billing';
            else moduleId = 'school';
          }
        }
      } catch (error) {
        this.logger.error(`Error resolving tenant from domain: ${error.message}`, error.stack, 'TenantMiddleware');
      }
    }

    // 3. Header Fallback (For manual API testing / legacy support)
    if (!franchiseId) {
      const franchiseIdHeader = req.headers['x-franchise-id'] || req.headers['x-tenant-id'];
    ...
    if (franchiseId) {
      this.logger.verbose(`Tenant Scope set to Franchise: ${franchiseId} | Module: ${moduleId || 'unknown'}`, 'TenantMiddleware');
    }

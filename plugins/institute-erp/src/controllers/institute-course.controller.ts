...
export class InstituteCourseController {
  ...
  async getCourses() {
    const res = await this.dataSource.query('SELECT * FROM courses ORDER BY id DESC');
  ...
  async createCourse(@Body() data: any) {
    await this.dataSource.query(`INSERT INTO courses (course_name, fees, total_fees) VALUES (?, ?, ?)`, [data.course_name, data.fees || 0, data.total_fees || 0]);
